...
def process_resources(data_dict, user_obj):
    ...
    if not is_sysadmin:
        ...
        if not has_user_permission_for_org(data_dict.get('owner_org'),
                                           user_obj, 'read'):
            ...
            for resource in list(resources):
                hide_resource = resource.get('resource_visible',
                                             'TRUE') == 'FALSE'
                if not hide_resource:
                    governance_acknowledgement = resource.get(
